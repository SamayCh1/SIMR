...
def rotate_block_90_cw(pytromino, pos):
    ...
    # BEGIN QUESTION 6
    return (pytromino.center_rot[1] - pos[1] + pytromino.center_rot[0], pos[0] - pytromino.center_rot[0] + pytromino.center_rot[1])
    # END QUESTION 6
...
def filter_blocks_pos(pytromino, fn):
    ...
    # BEGIN QUESTION 7
    newblocks = []
    for item in pytromino.blocks_pos: 
        if fn(item):
...
def validated_apply_non_rot(pytromino, fn, validator):
    ...
    # BEGIN QUESTION 10
    valid = []
    for item in new_pytro.blocks_pos:
        valid.append(validator(fn(item)))
    valid.append(validator(fn(new_pytro.center_rot)))
    if all(valid):
        for i in range(len(new_pytro.blocks_pos)):
            # print(fn(new_pytro.blocks_pos[i]) )
            new_pytro.blocks_pos[i] = fn(new_pytro.blocks_pos[i]) 
        new_pytro.center_rot = fn(new_pytro.center_rot) 
